...
The host creates a room, picks the team names, the scale categories (general or analytics), the target score, and whether side bets are on. Players join with the four-character room code and pick a team. At least two teams need **two** players each before the game can start, and nobody may be left alone in a team — the clue-giver doesn't place a marker, so a team of one has a clue and nobody to answer it, which is a round that cannot resolve. The lobby names the teams that are short, shows the shortfall next to each team in the picker, and keeps Start disabled until the room is playable; `startGame` enforces the same rule server-side for a stale client or a direct API call. Mid-game the rotation skips a team that has shrunk below two, falling back to any non-empty team only if nothing else is playable — a stalled round the host can reveal beats a room that cannot advance at all. `MIN_TEAM_SIZE` in `src/lib/game/engine.ts` is the single place the number lives.

Each round picks a scale that hasn't been used yet in that room and a secret target between 5 and 95. The catalogue holds 262 pairs — about 180 general and 82 analytics — so a long evening never repeats itself. The clue-giver on the active team is whoever has given the fewest clues so far, ties broken by join order. They see the target; nobody else does. They write a clue under the rules below. Their teammates then each drag their own slider and lock it in, without seeing each other's. The other teams see the clue and watch the markers appear on the dial, which is what their left-or-right side bet is a judgement about. Once everyone who can act has acted the round reveals automatically, and the host or clue-giver can force the reveal early.
...
`src/app` holds the pages and the API route handlers. `src/lib/game/engine.ts` is the pure rules — scoring, averaging, bet resolution, rotation, target and code generation, input sanitising — and has no I/O, which is why it is the part under test. `src/lib/game/clue.ts` is the clue rules, pure for the same reason and shared by the input box and the server so the two cannot drift; `src/lib/game/clue-words.ts` is the word data it reads, kept separate so the lists can be edited without reading the algorithm. `src/lib/server` is the backend service layer that owns every write, including `scales.ts`, the cached database-backed pool, and the two per-viewer reads — `getSecretTarget` and `getLiveGuesses` — that answer things the shared room state deliberately cannot. `src/lib/server/analytics.ts` is the event ingest and the report, `src/lib/server/mixpanel.ts` the Mixpanel mirror, and `src/lib/server/schema-check.ts` what lets `/api/health` name a missing table. `src/lib/client` is the sync hook, the fetch wrappers, the localStorage identity, and `track.ts`, the batching event queue. `src/lib/demo/script.ts` and `src/components/demo` are the scripted tutorial behind `/how-to-play` — the step list and the player that walks it — and are the one part of the tree that talks to nothing at all. `src/lib/scales.ts` is types and pure helpers; `src/lib/scales-data.ts` is the 262-pair catalogue, keyed by stable identifiers so labels can be reworded without orphaning old stats. `src/lib/player-tag.ts` is the four-character device tag, pure and shared so the browser can hash its own id and recognise its own leaderboard row without the server ever learning which row that is. `src/lib/i18n.ts` is a flat UA/EN dictionary shared by both sides. `scripts/gen-scales-sql.ts` generates the seed. `supabase/schema.sql` is the whole database and `supabase/scales-seed.sql` its catalogue.
